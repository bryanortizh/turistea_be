...
import {
  findAdminByEmail,
  findOneAdmin,
} from "../../admin/services/find/admin";
import {
  validateStatusAdmin,
  amountIntAdmin,
  ...
  try {
    ...
    if (_validateStatusUser) {
      ...
      if (_validatePassPriv) {
        ...
        throw createError(
          401,
          "No tiene permiso, por favor ingrese nuevamente"
        );
      }
    } else {
      if (user.status == "BI") {
      ...
      if (user.status == "BIT") {
        ...
        var segRestantesv2 = Math.round(
          Number("0." + minRestantes.toString().split(".")[1]) * 60
        );
        ...
        throw createError(
          ...
          `Intentelo nuevamente en ${
            Math.floor(minRestantes) == 0
              ? ""
              : `${Math.floor(minRestantes)} minuto${
                  Math.floor(minRestantes) != 1 ? "s" : ""
                } y `
          }${segRestantesv2} segundo${
            Math.floor(segRestantesv2) != 1 ? "s" : ""
          }`
        );
      }
    }
...
const validatePassPriv = ({
  ...
  const salt2 = CryptoJS.lib.WordArray.random(30);
  const hashpwd2 = CryptoJS.PBKDF2("12345", salt2.toString(), {
    iterations: 10000,
    keySize: 10,
  });
...
export const signInAdminService = async ({
  ...
  try {
    ...
    if (_validateStatusAdmin) {
      ...
      if (_validatePassPriv) {
        if (admin!.numIntentos! > 0)
          await desbloqueoAdmin({ userId: admin!.id });
      ...
      if (admin!.status == "BIT") {
        ...
        new Date(
          fechaActual.setTime(
            Number(
              fechaActual.getTime() + 60000 * admin!.cantidad_min_bloqueado!
            )
          )
        );
        var diff = Math.abs(Number(fechaActual) - Number(new Date()));
