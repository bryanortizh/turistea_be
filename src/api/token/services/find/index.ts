...
export const gethoursOfUseInTheWeek = async ({
  weekDate,
  yearDate,
  monthDate,
}: {
    ...
    const data = await DataBase.instance.token.findAll({
        ...
        [
          sequelize.fn(
            ...
            sequelize.fn(
              "sum",
              sequelize.literal(
                "TIMESTAMPDIFF(second,start_session, last_session)"
              )
            ),
            0
          ),
        ...
        [
          sequelize.fn(
            ...
            sequelize.fn(
              "sum",
              sequelize.literal(
                "TIMESTAMPDIFF(minute,start_session, last_session)"
              )
            ),
            0
          ),
        ...
        [
          sequelize.fn(
            ...
            sequelize.fn(
              "sum",
              sequelize.literal(
                "TIMESTAMPDIFF(hour,start_session, last_session)"
              )
            ),
            0
          ),
      ...
      where: {
        ...
        [Op.and]: [
          ...
          sequelize.where(
            sequelize.fn("YEAR", sequelize.col("start_session")),
            yearDate
          ),
          sequelize.where(
...
export const gethoursOfUseInTheMonth = async ({
  weekDate,
  yearDate,
  monthDate,
}: {
  ...
  try {
    ...
    const useFrecuencyReport = await DataBase.instance.token.findAll({
      attributes: [
        ...
        [
          sequelize.literal(
            `round(count(*)/cast(TIMESTAMPDIFF(day, '${startDate}' , '${finishDate}') as signed),2)`
          ),
          "day",
        ],
        [
          sequelize.literal(
            `round(count(*)/(cast(TIMESTAMPDIFF(day, '${startDate}' , '${finishDate}') as signed)/7),2)`
          ),
          "week",
        ],
        [
          sequelize.literal(
            `round(count(*)/(cast(TIMESTAMPDIFF(day, '${startDate}' , '${finishDate}') as signed)/30),2)`
          ),
          "month",
        ],
        [
          sequelize.literal(
            `sec_to_time(sum(TIMESTAMPDIFF(second,start_session, last_session)))`
          ),
          "usetime",
        ],
        [
